...
// Function to insert a record into the database (sorted by roll number)
void insert(char name[], int rollNumber, float percentage) {
    // If the database is full, return
    if (numRecords >= MAX_RECORDS) {
        printf("Database is full. Cannot insert more records.\n");
        return;
    }

    // Find the position to insert the record (sorted by roll number)
    int i;
    for (i = numRecords - 1; i >= 0 && studentDB[i].rollNumber > rollNumber; i--) {
        studentDB[i + 1] = studentDB[i]; // Shift records to the right
    }
...
// Function to find one name from the database which starts with the given prefix
char* findOne(char prefix[]) {
    ...
    return NULL; // If no matching name found
}

// Function to calculate the count, highest percentage, lowest percentage, mean, and standard deviation
...
// Function to delete a record with the given roll number
